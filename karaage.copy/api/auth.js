...
module.exports = async (req, res) => {
  try {
    ...
    const cookies = req.cookies || (req.headers && req.headers.cookie ? cookie.parse(req.headers.cookie) : {});
    ...
    console.error('Global auth handler error:', globalErr);
    return res.status(500).json({ error: globalErr.message || 'Internal Server Error in auth handler' });
  }
};
